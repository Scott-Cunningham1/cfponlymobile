...
function ScoreDash() {
  ...
  const maxWidth = SCREEN_WIDTH * 0.4; // Set your max width as a percentage of the screen width
  const minFontSize = 10;

  useEffect(() => {
    const adjustFontSize = () => {
      const textWidth = maxWidth; // Assume maximum width for calculation
      while (textWidth < maxWidth && fontSize > minFontSize) {
        setFontSize((prevSize) => Math.max(prevSize - 1, minFontSize));
      }
    };
    ...
    return (
      ...
      <TouchableOpacity onPress={() => handleCardPress(item)} style={styles.horizontalCardContent}>
        <View style={styles.leftColumn}>
          ...
          <Text style={[styles.horizontalCardText, { fontSize }]}numberOfLines={1} ellipsizeMode="tail">{item.away}</Text>
        ...
        <View style={styles.rightColumn}>
          ...
          <Text style={[styles.horizontalCardText, { fontSize }]}>{item.home}</Text>
    ...
    <View style={[styles.leagueGamesCard, { backgroundColor }]}>
      <TouchableOpacity onPress={() => handleCardPress(item)} style={styles.horizontalCardContent}>
        <View style={styles.leftColumn}>
          <Text style={styles.leftColumnText}>{item.away_user || "Open"}</Text>
          <Image style={styles.image} source={getImageSource(item.away)} />
          <Text style={[styles.horizontalCardText, { fontSize }]}>{item.away}</Text>
        </View>
