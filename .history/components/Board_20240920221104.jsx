...
import { View, TextInput, Button, FlatList, Text, StyleSheet, Alert, Dimensions } from 'react-native';
...
useEffect(() => {
  if (initialRender && flatListRef.current) {
    ...
    setInitialRender(false); // Set flag to false after first scroll
  }
...
const MessageList = ({ messages }) => {
  const textColor = if(messages.username === 'jamal' ? 'red' : 'black'
  return (
    <FlatList
      data={messages}
      renderItem={({ item }) => (
        <View style={styles.messageItem}>
          <Text style={[styles.userText, { color: textColor }]}>{item.username}:</Text>
...
const styles = StyleSheet.create({
  ...
  userText: {
    ...
    color: '#fe813b', // Use color instead of textColor
    fontWeight: 'bold',
  },
