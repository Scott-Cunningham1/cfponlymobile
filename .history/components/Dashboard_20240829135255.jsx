  ...
  const renderHorizontalCard = ({ item }) => (
    <View style={styles.horizontalCard}>
      <TouchableOpacity onPress={() => handleCardPress(item)} style={styles.horizontalCardContent}>
        <Text style={styles.horizontalCardText}>{item.username}</Text>
      </TouchableOpacity>
    </View>
  );
...
<ScrollView >
      ...
      />
      <FlatList
        data={leaderboard.slice(0, 3)} // Display a subset for horizontal scroll, adjust as needed
        renderItem={renderHorizontalCard}
        keyExtractor={(item) => item.home_team}
        horizontal
