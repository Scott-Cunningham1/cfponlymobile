...
function ScoreDash() {
  ...
  const getDashInfo = useCallback(async () => {
    try{
      ...
      const userLinesData = await getUserLines(user.id, week);
  ...
  useEffect(() => {
    ...
    if (teams.length > 0 && leaderboard.length > 0 && rankings.length > 0, lines.length > 0, userLines.length > 0) {
      setExecuting(false);
    }
  }, [teams, rankings, leaderboard, lines, userLines]);
