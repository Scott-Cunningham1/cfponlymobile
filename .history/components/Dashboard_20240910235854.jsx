...
function ScoreDash() {
  ...
  useEffect(() => {
    ...
    if (teams.length > 0 && rankings.length > 0 && leaderboard.length > 0 && userTeams.length > 0) {
      setExecuting(false);
    }
  }, [teams, rankings, leaderboard, userTeams]);
  ...
  const renderUserCard = ({ item }) => (
    <View style={styles.userGamesCard}>
      <TouchableOpacity onPress={() => handleCardPress(item)} style={styles.horizontalCardContent}>
        <Text style={styles.horizontalCardText}>source=<Image style={styles.image} source={getImageSource(item.home)} /> {item.home}</Text>
        <Text style={styles.horizontalCardText}><Image style={styles.image} source={getImageSource(item.away)} /> {item.away}</Text>
        <Text style={styles.horizontalCardDate}>{item.date}</Text>
      </TouchableOpacity>
    </View>
