...
function ScoreDash() {
  ...
  const [updatedGamesArray, setUpdatedGamesArray] = useState([]);
    ...
    const getDashInfo = useCallback(async () => {
      try {
        ...
        setTeams(teamsData);
        const userTeamsData = await getUserTeams(tokenData, userId);
        ...
        for (const item in games) {
          if(userTeams.includes(item.home_team) || userTeams.includes(item.away_team)) {
            userGamesData.push(item)
          }
        }
        ...
        userGames.forEach(game => {
            ...
            const homeTeamRank = rankings.find(ranking => ranking.name === game.home_team);
   
            const awayTeamRank = rankings.find(ranking => ranking.name === game.away_team);

            const newGame = {
                home_team: game.home_team,
                home_rank: homeTeamRank ? homeTeamRank.rank : " ",
                away_team: game.away_team,
                away_rank: awayTeamRank ? awayTeamRank.rank : " "
            };


    updatedGames.push(newGame);
    setUpdatedGamesArray(updatedGames)
        ...
        console.log(updatedGamesArray)
        
        if (updatedGamesArray) {
          setLoading(false);
}
      } catch (error) {
