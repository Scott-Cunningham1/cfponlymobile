...
function ScoreDash() {
  ...
  const renderLeagueCard = ({ item }) => {
      let homeTeam, homeCode, awayTeam, awayCode;
      if (item.home.includes('-')) {
      const [homeTeam, homeCode] = item.home.split(' -');
      const [awayTeam, awayCode] = item.away.split(' o');
      }
      else {
        const [homeTeam, homeCode] = item.home.split(' o');
        const [awayTeam, awayCode] = item.away.split(' -');
      }
      homeCode = homeCode ? `f${homeCode}` : '';
      awayCode = awayCode ? `f${awayCode}` : '';
    ...
    return (
      ...
      <TouchableOpacity onPress={() => handleCardPress(item)} style={styles.horizontalCardContent}>
        <View style={styles.leftColumn}>
          ...
          <Text style={styles.horizontalCardText}>{awayTeam}</Text>
          <Text style={styles.horizontalCardText}>{awayCode}</Text>
        </View>
        ...
        <View style={styles.rightColumn}>
          ...
          <Text style={styles.horizontalCardText}>{homeTeam}</Text>
          <Text style={styles.horizontalCardText}>{homeCode}</Text>
        </View>
      </TouchableOpacity>
    </View>
    )
  ...
  const renderLeaguesCard = ({ item }) => {
    ...
    const backgroundColor = item.home_user && item.away_user ? '#d1e7dd' : '#ffffff'; // Example condition
    return (
