...
function Individual() {
  ...
  const [userLines, getuserLines] = useState([]);
  const navigation = useNavigation();
    ...
    const getInfo = useCallback( async () => {
      try {
        ...
        const userLinesData = await getUserTeams(tokenData, user.id);
        setUserTeams(userTeamsData);
        console.log("userTeams:", userTeams)
  ...
  const TopCard = () => (
    <View style={[styles.topCard, {backgroundColor: 'rgba(255, 255, 255, 0)'}]}>
      <Text style={styles.topCardText}>{user.username || 'User'} owes {teams.losses.reduce((total, current) => total + current, 0)}</Text>
    </View>
  );
