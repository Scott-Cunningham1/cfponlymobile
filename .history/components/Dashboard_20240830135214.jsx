...
function ScoreDash() {
  ...
  const getDashInfo = useCallback(async () =>{
    ...
    setUser(userData);
    const userTeamsData = await getUserTeams(token, user.id);
    setUserTeams(userTeamsData);
    const teamsData = await getUser(token);
    setTeams(teamsData);
    const gamesData = await getGames(token);
    setGames(gamesData);
    const rankingsData = await getRankings(token, week);
    setRankings(rankingsData);
    const leaderboardData = await getUser(token);
    setTeams(teamsData);
    const gamesData = await getGames(token);
    setGames(gamesData);
    ...
    const getDashInfo = useCallback(async () => {
      try {
        ...
        function fetchDependentData(state) {
          return new Promise(resolve => {
            setTimeout(() => resolve(`dependentData based on ${state}`), 1000);
          });
        }
        
        // Execute functions in order
        fetchPageData()
          .then(data => setState(data))           // Step 1: Get Data, Step 2: Set State with Data
          .then(state => fetchDependentData(state)) // Step 3: Get other Data
          .then(otherData => {
