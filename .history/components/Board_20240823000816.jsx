...
import getBoard from '../common/getTeams';
  ...
  const navigation = useNavigation();
    ...
    const getInfo = useCallback( async () => {
      try {
        ...
        const teamsWithUsernames = teams
          .filter(team => team.user_id !== 0)
          .map(team => {
        const userObj = user.find(user => user.id === team.user_id);
        return { ...team, username: userObj ? userObj.username : 'Unknown' };
      });
